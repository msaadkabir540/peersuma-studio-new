...
import { EmbedWidgetThumbInterface } from "./embed-widget-thumb-interface";
...
function EmbedWidgetThumb({
  ...
  useMemo(() => {
    const minutes = Math.floor(item.duration || 60 / 60);
    const seconds = (item.duration || 60) - minutes * 60;
    const strTime = minutes.toString() + ":" + seconds.toString().padStart(2, "0");
    setFullTime(strTime);
  }, [item]);

  const uploadedTime = moment(item?.updatedAt).format("MMM DD, YYYY | HH:mm");

  const uploadedByUserName = item?.userId?.fullName || item?.userId?.username || "Superadmin";
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ order: "1", color: thumbnailTitleColor, background: thumbnailColor }}
      ...
      <div
        ...
        style={{
          ...
          border: `2px solid ${thumbnailColor}`,
          borderRadius: "0px 0px 5px 5px",
        }}
      >
        {/*Image*/}
        <div className={style.embedWidgetThumbImageContainer} aria-label={item.name}>
          <img
        ...
        {!selected && (
          <>
            <div className={style.embedWidgetThumbTimeContainer}>{fullTime}</div>
      ...
      <div className={style.uploadedContainer}>
        <div className={style.uploadedNameClass}>
          <div style={{ backgroundColor: avatarColor, color: textColor }} className={style.avatar}>
            {uploadedByUserName?.charAt(0)}
          </div>
          <div style={{ color: avatarColor }}>
